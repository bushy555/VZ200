...
_s40:
	...
	;DEFINE	KEMPSTON_TEST	; do we want to check for the Kempston too?
...
; =====================================================================================================================
;  this version of the core is somewhat lacking in bass and slightly noisier, yet i quite like its sound
;
;					default version			"slower" version
;  (*) discretization frequency:	3500000/128 ~ 27.3kHz		3500000/136 ~ 25.7kHz
;  (*) relative channel durations:	88 / 40				88 / 48
;  (*) relative channel volumes:	1:40/88 = 1:0.455		1:48/88 = 1:0.545
;
; =====================================================================================================================
...
;
;  there are also three paths through the second half of the new main loop:
;  (6) -> (7)		->	4+8 + 4+(11+4+11+4+8+10) + 11 = 75t / 48t	(output changes)
;  (6) -> (8) -> (9)	->	4+13 + 7+(11+4+4+4+7 + 10) + 11 = 75t / 40t	(output stays the same)
;  (6) -> (8) -> (10)	->	4+13 + 7+11+4+4+4+12 + 6+10 = 75t		(special fx for channel 2)
;

MainLoopPt2:	exx			; 4t		(6)
		djnz	ContChannel2	; 13t/8t
...
ContChannel2:	or	0		; 7t		(8)
		ld	($6800), a
;		out	(254), a	; 11t
		ex	af, af'		; 4t
		rlca			; 4t
		rrca			; 4t
		jr	c, FXChannel2	; 12t/7t

		jp	PreMainLoop	; 10t		(9)

FXChannel2:	dec	hl		; 6t		(10)
		jp	MainLoopPt1	; 10t
...
;              XOR   $10			; 7+11+4+8+4+4+4+4+4+4+4+12-5=65
		xor	$21
LEA30:        ;OUT   ($FE),A			; 11
		ld	($6800), a		; 13
              LD    E,A
